...
TOPIC = os.getenv("KAFKA_VALIDATED_TOPIC", "market.ticks.validated")
GROUP_ID = os.getenv("KAFKA_GROUP_ID", f"debug-validated-{int(time.time())}")
...
def ensure_topic() -> None:
    ...
    admin = KafkaAdminClient(bootstrap_servers=BOOTSTRAP_SERVER, client_id="debug-validated-admin")
    try:
...
def main() -> None:
    ...
    try:
        for msg in consumer:
            ...
            print(json.dumps({"partition": msg.partition, "offset": msg.offset, "value": value}))
    finally:
        consumer.close()
