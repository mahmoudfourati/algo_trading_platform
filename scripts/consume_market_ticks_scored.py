...
TOPIC = os.getenv("KAFKA_SCORED_TOPIC", "market.ticks.scored")
GROUP_ID = os.getenv("KAFKA_GROUP_ID", f"debug-scored-{int(time.time())}")
...
def ensure_topic() -> None:
    ...
    admin = KafkaAdminClient(bootstrap_servers=BOOTSTRAP_SERVER, client_id="debug-scored-admin")
    try:
